...
export class VehiclesService {
  ...
  async createVehicle(createVehicleDto: CreateVehicleDto): Promise<Vehicle> {
    ...
    const yearOption = await this.yearOptionRepository.findOneBy({
      id: createVehicleDto.yearOptionId,
    });
    ...
    if (!yearOption)
      throw new NotFoundException(`YearOption with ID ${createVehicleDto.yearOptionId} not found`);
  ...
  async updateVehicle(id: string, updateVehicleDto: UpdateVehicleDto): Promise<Vehicle> {
    ...
    if (updateVehicleDto.brandId) {
      ...
      if (!brand)
        throw new NotFoundException(`Brand with ID ${updateVehicleDto.brandId} not found`);
      vehicle.brand = brand;
    }
    ...
    if (updateVehicleDto.modelId) {
      ...
      if (!model)
        throw new NotFoundException(`Model with ID ${updateVehicleDto.modelId} not found`);
      vehicle.model = model;
    }

    if (updateVehicleDto.yearOptionId) {
      const yearOption = await this.yearOptionRepository.findOneBy({
        id: updateVehicleDto.yearOptionId,
      });
      if (!yearOption)
        throw new NotFoundException(
          `YearOption with ID ${updateVehicleDto.yearOptionId} not found`,
        );
